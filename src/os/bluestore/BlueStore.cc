...
static void get_coll_range(const coll_t& cid, int bits, ghobject_t* temp_start, ghobject_t* temp_end, ghobject_t* start, ghobject_t* end, bool legacy) {
    spg_t pgid;
...
static void get_object_key(CephContext* cct, const ghobject_t& oid, S* key) {
    ...
    size_t max_len = ENCODED_KEY_PREFIX_LEN + (oid.hobj.nspace.length() * 3 + 1) + (oid.hobj.get_key().length() * 3 + 1) + 1 +  // for '<', '=', or '>'
...
// of the prefix bytes.
template <typename S>
static void get_extent_shard_key(const S& onode_key, uint32_t offset, string* key) {
    key->clear();
...
template <typename S>
static void generate_extent_shard_key_and_apply(const S& onode_key, uint32_t offset, string* key, std::function<void(const string& final_key)> apply) {
...
static bool is_extent_shard_key(const string& key) { return *key.rbegin() == EXTENT_SHARD_KEY_SUFFIX; }

static void get_deferred_key(uint64_t seq, string* out) { _key_encode_u64(seq, out); }
...
void _dump_onode(CephContext* cct, const BlueStore::Onode& o) {
    ...
    dout(LogLevelV) << __func__ << " " << &o << " " << o.oid << " nid " << o.onode.nid << " size 0x" << std::hex << o.onode.size << " (" << std::dec << o.onode.size << ")"
                    << " expected_object_size " << o.onode.expected_object_size << " expected_write_size " << o.onode.expected_write_size << " in " << o.onode.extent_map_shards.size() << " shards"
...
ostream& operator<<(ostream& out, const BlueStore::Buffer& b) {
    out << "buffer(" << &b << " space " << b.space << " 0x" << std::hex << b.offset << "~" << b.length << std::dec << " " << BlueStore::Buffer::get_state_name(b.state);
...
class SimpleCollectionListIterator : public CollectionListIterator {
   public:
    SimpleCollectionListIterator(CephContext* cct, const KeyValueDB::Iterator& it) : CollectionListIterator(it), m_cct(cct) {}
...
class SortedCollectionListIterator : public CollectionListIterator {
   public:
    SortedCollectionListIterator(const KeyValueDB::Iterator& it) : CollectionListIterator(it), m_chunk_iter(m_chunk.end()) {}
    ...
    void lower_bound(const ghobject_t& oid) override {
        ...
        if (this->oid().shard_id != oid.shard_id || this->oid().hobj.pool != oid.hobj.pool || this->oid().hobj.get_bitwise_key_u32() != oid.hobj.get_bitwise_key_u32()) {
            return;
        }
   ...
   private:
    ...
    bool get_next_chunk() {
        ...
        while (true) {
            ...
            ceph_assert(r == 0);
            if (next.shard_id != oid.shard_id || next.hobj.pool != oid.hobj.pool || next.hobj.get_bitwise_key_u32() != oid.hobj.get_bitwise_key_u32()) {
                break;
            }
            oid = next;
        }
...
void BlueStore::GarbageCollector::process_protrusive_extents(const BlueStore::ExtentMap& extent_map, uint64_t start_offset, uint64_t end_offset, uint64_t start_touch_offset, uint64_t end_touch_offset,
    ...
    for (auto it = extent_map.seek_lextent(lookup_start_offset); it != extent_map.extent_map.end() && it->logical_offset < lookup_end_offset; ++it) {
...
int64_t BlueStore::GarbageCollector::estimate(uint64_t start_offset, uint64_t length, const BlueStore::ExtentMap& extent_map, const BlueStore::old_extent_map_t& old_extents, uint64_t min_alloc_size) {
    ...
    if (gc_start_offset < start_offset || gc_end_offset > end_offset) {
        process_protrusive_extents(extent_map, gc_start_offset, gc_end_offset, start_offset, end_offset, min_alloc_size);
    }
...
struct LruOnodeCacheShard : public BlueStore::OnodeCacheShard {
    typedef boost::intrusive::list<BlueStore::Onode, boost::intrusive::member_hook<BlueStore::Onode, boost::intrusive::list_member_hook<>, &BlueStore::Onode::lru_item>> list_t;
...
struct LruBufferCacheShard : public BlueStore::BufferCacheShard {
    typedef boost::intrusive::list<BlueStore::Buffer, boost::intrusive::member_hook<BlueStore::Buffer, boost::intrusive::list_member_hook<>, &BlueStore::Buffer::lru_item>> list_t;
    list_t lru;
...
void BlueStore::BufferSpace::read(BufferCacheShard* cache, uint32_t offset, uint32_t length, BlueStore::ready_regions_t& res, interval_set<uint32_t>& res_intervals, int flags) {
    res.clear();
    res_intervals.clear();
    ...
    while (true) {
        ...
        if (p->second->offset < pos) {
            ...
            if (p->second->data.length()) {
                ...
                r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, 0, bl, p->second->flags), 0, p->second.get());
            } else {
                r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, 0, right, p->second->flags), 0, p->second.get());
            }
        ...
        if (p->second->data.length()) {
            r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->data, p->second->flags), 0, p->second.get());
        } else {
            r._add_buffer(cache, new Buffer(&r, p->second->state, p->second->seq, p->second->offset - pos, p->second->length, p->second->flags), 0, p->second.get());
        }
...
BlueStore::OnodeRef BlueStore::OnodeSpace::add_onode(const ghobject_t& oid, OnodeRef& o) {
    ...
    if (!p.second) {
        ldout(cache->cct, 30) << __func__ << " " << oid << " " << o << " raced, returning existing " << p.first->second << dendl;
...
BlueStore::OnodeRef BlueStore::OnodeSpace::lookup(const ghobject_t& oid) {
...

    {
        ...
        if (p == onode_map.end()) {
            ...
            ldout(cache->cct, 30) << __func__ << " " << oid << " hit " << p->second << " " << p->second->nref << " " << p->second->cached << dendl;
...
void BlueStore::OnodeSpace::rename(OnodeRef& oldo, const ghobject_t& old_oid, const ghobject_t& new_oid, const mempool::bluestore_cache_meta::string& new_okey) {
    std::lock_guard l(cache->lock);
...
void BlueStore::OnodeSpace::dump(CephContext* cct) {
    for (auto& i : onode_map) {
        ldout(cct, LogLevelV) << i.first << " : " << i.second << " " << i.second->nref << " " << i.second->cached << dendl;
    }
}
...
BlueStore::SharedBlob::SharedBlob(uint64_t i, Collection* _coll) : coll(_coll), sbid_unloaded(i) {
...
bool BlueStore::Blob::can_reuse_blob(uint32_t min_alloc_size, uint32_t target_blob_size, uint32_t b_offset, uint32_t* length0) {
    ...
    if (get_blob().has_csum() && ((b_offset % get_blob().get_csum_chunk_size()) != 0 || (end % get_blob().get_csum_chunk_size()) != 0)) {
        return false;
    }
...
void BlueStore::Blob::decode(Collection* coll, bufferptr::const_iterator& p, uint64_t struct_v, uint64_t* sbid, bool include_ref_map) {
...
ostream& operator<<(ostream& out, const BlueStore::Extent& e) {
    return out << std::hex << "0x" << e.logical_offset << "~" << e.length << ": 0x" << e.blob_offset << "~" << e.length << std::dec << " " << *e.blob;
}
...
BlueStore::ExtentMap::ExtentMap(Onode* o) : onode(o), inline_bl(o->c->store->cct->_conf->bluestore_extent_map_inline_shard_prealloc_size) {}
...
void BlueStore::ExtentMap::dup(BlueStore* b, TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t& srcoff, uint64_t& length, uint64_t& dstoff) {
    ...
    for (auto ep = oldo->extent_map.seek_lextent(srcoff); ep != oldo->extent_map.extent_map.end(); ++ep) {
        ...
        Extent* ne = new Extent(e.logical_offset + skip_front + dstoff - srcoff, e.blob_offset + skip_front, e.length - skip_front - skip_back, cb);
    ...
    if (onode->onode.extent_map_shards.empty()) {
        ...
        while (p != shards.end()) {
            ...
            if (p->dirty) {
                ...
                dout(20) << __func__ << "  shard 0x" << std::hex << p->shard_info->offset << std::dec << " is " << len << " bytes (was " << p->shard_info->bytes << ") from " << p->extents
                         << " extents" << dendl;
        ...
        for (auto& it : encoded_shards) {
            ...
            generate_extent_shard_key_and_apply(onode->key, it.shard->shard_info->offset, &key, [&](const string& final_key) { t->set(PREFIX_OBJ, final_key, it.bl); });
        }
    }
}
...
void BlueStore::ExtentMap::reshard(KeyValueDB* db, KeyValueDB::Transaction t) {
    ...
    dout(10) << __func__ << " 0x[" << std::hex << needs_reshard_begin << "," << needs_reshard_end << ")" << std::dec << " of " << onode->onode.extent_map_shards.size() << " shards on " << onode->oid
             << dendl;
    ...
    if (!shards.empty()) {
        ...
        dout(20) << __func__ << "   shards [" << si_begin << "," << si_end << ")"
                 << " over 0x[" << std::hex << needs_reshard_begin << "," << needs_reshard_end << ")" << std::dec << dendl;
    }
    ...
    for (unsigned i = si_begin; i < si_end; ++i) {
        generate_extent_shard_key_and_apply(onode->key, shards[i].shard_info->offset, &key, [&](const string& final_key) { t->rmkey(PREFIX_OBJ, final_key); });
    }
    ...
    if (sv.empty()) {
            ...
            if (e->blob_escapes_range(shard_start, shard_end - shard_start)) {
                if (!e->blob->is_spanning()) {
                    ...
                    if (must_span) {
                        ...
                        if (!was_too_many_blobs_check && too_many_blobs_threshold && spanning_blob_map.size() >= size_t(too_many_blobs_threshold)) {
        ...
        bool do_dump = (!oid_slot && was_too_many_blobs_check) || (oid_slot && (mono_clock::now() - oid_slot->second >= make_timespan(5 * 60)));
        if (do_dump) {
...
int BlueStore::ExtentMap::compress_extent_map(uint64_t offset, uint64_t length) {
    ...
    for (++n; n != extent_map.end(); p = n++) {
        ...
        while (n != extent_map.end() && p->logical_end() == n->logical_offset && p->blob == n->blob && p->blob_offset + p->length == n->blob_offset && n->logical_offset < shard_end) {
...
BlueStore::Extent* BlueStore::ExtentMap::set_lextent(CollectionRef& c, uint64_t logical_offset, uint64_t blob_offset, uint64_t length, BlobRef b, old_extent_map_t* old_extents) {
...
BlueStore::Onode* BlueStore::Onode::create_decode(CollectionRef c, const ghobject_t& oid, const string& key, const bufferlist& v, bool allow_empty) {
...
void BlueStore::DeferredBatch::prepare_write(CephContext* cct, uint64_t seq, uint64_t offset, uint64_t length, bufferlist::const_iterator& blp) {
...
void BlueStore::DeferredBatch::_discard(CephContext* cct, uint64_t offset, uint64_t length) {
    ...
    while (p != iomap.end()) {
        ...
        if (end > offset + length) {
            ...
            dout(20) << __func__ << "  truncate front " << p->second.seq << " 0x" << std::hex << p->first << "~" << p->second.bl.length() << " drop_front 0x" << drop_front << " keep_tail 0x"
                     << keep_tail << " to 0x" << (offset + length) << "~" << keep_tail << std::dec << dendl;
...
BlueStore::Collection::Collection(BlueStore* store_, OnodeCacheShard* oc, BufferCacheShard* bc, coll_t cid)
    : CollectionImpl(store_->cct, cid), store(store_), cache(bc), exists(true), onode_space(oc), commit_queue(nullptr) {}

bool BlueStore::Collection::flush_commit(Context* c) { return osr->flush_commit(c); }
...
void BlueStore::Collection::load_shared_blob(SharedBlobRef sb) {
    if (!sb->is_loaded()) {
        ...
        if (r < 0) {
            lderr(store->cct) << __func__ << " sbid 0x" << std::hex << sbid << std::dec << " not found at key " << pretty_binary_string(key) << dendl;
...
BlueStore::OnodeRef BlueStore::Collection::get_onode(const ghobject_t& oid, bool create, bool is_createop) {
    ...
    if (cid.is_pg(&pgid)) {
        if (!oid.match(cnode.bits, pgid.ps())) {
            lderr(store->cct) << __func__ << " oid " << oid << " not part of " << pgid << " bits " << cnode.bits << dendl;
            ceph_abort();
        }
    }
...
void BlueStore::MempoolThread::_resize_shards(bool interval_stats) {
    ...
    if (interval_stats) {
        dout(5) << __func__ << " cache_size: " << cache_size << " kv_alloc: " << kv_alloc << " kv_used: " << kv_used << " kv_onode_alloc: " << kv_onode_alloc << " kv_onode_used: " << kv_onode_used
                << " meta_alloc: " << meta_alloc << " meta_used: " << meta_used << " data_alloc: " << data_alloc << " data_used: " << data_used << dendl;
    } else {
        dout(20) << __func__ << " cache_size: " << cache_size << " kv_alloc: " << kv_alloc << " kv_used: " << kv_used << " kv_onode_alloc: " << kv_onode_alloc << " kv_onode_used: " << kv_onode_used
                 << " meta_alloc: " << meta_alloc << " meta_used: " << meta_used << " data_alloc: " << data_alloc << " data_used: " << data_used << dendl;
    }

    uint64_t max_shard_onodes = static_cast<uint64_t>((meta_alloc / (double)onode_shards) / meta_cache->get_bytes_per_onode());
...
BlueStore::OmapIteratorImpl::OmapIteratorImpl(CollectionRef c, OnodeRef& o, KeyValueDB::Iterator it) : c(c), o(o), it(it) {
...
int BlueStore::OmapIteratorImpl::seek_to_first() {
    ...
    c->store->log_latency(__func__, l_bluestore_omap_seek_to_first_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
int BlueStore::OmapIteratorImpl::upper_bound(const string& after) {
    ...
    c->store->log_latency_fn(__func__, l_bluestore_omap_upper_bound_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age,
...
int BlueStore::OmapIteratorImpl::lower_bound(const string& to) {
    ...
    c->store->log_latency_fn(__func__, l_bluestore_omap_lower_bound_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age,
...
int BlueStore::OmapIteratorImpl::next() {
    ...
    c->store->log_latency(__func__, l_bluestore_omap_next_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
BlueStore::BlueStore(CephContext* cct, const string& path) : BlueStore(cct, path, 0) {}
...
void BlueStore::handle_conf_change(const ConfigProxy& conf, const std::set<std::string>& changed) {
    ...
    if (changed.count("bluestore_compression_mode") || changed.count("bluestore_compression_algorithm") || changed.count("bluestore_compression_min_blob_size") ||
        changed.count("bluestore_compression_max_blob_size")) {
        if (bdev) {
            _set_compression();
        }
    }
    if (changed.count("bluestore_max_blob_size") || changed.count("bluestore_max_blob_size_ssd") || changed.count("bluestore_max_blob_size_hdd")) {
        if (bdev) {
            // only after startup
            _set_blob_size();
        }
    }
    if (changed.count("bluestore_prefer_deferred_size") || changed.count("bluestore_prefer_deferred_size_hdd") || changed.count("bluestore_prefer_deferred_size_ssd") ||
        changed.count("bluestore_max_alloc_size") || changed.count("bluestore_deferred_batch_ops") || changed.count("bluestore_deferred_batch_ops_hdd") ||
    ...
    if (changed.count("bluestore_throttle_cost_per_io") || changed.count("bluestore_throttle_cost_per_io_hdd") || changed.count("bluestore_throttle_cost_per_io_ssd")) {
        if (bdev) {
            _set_throttle_params();
        }
    }
    if (changed.count("bluestore_throttle_bytes") || changed.count("bluestore_throttle_deferred_bytes") || changed.count("bluestore_throttle_trace_rate")) {
    ...
    if (changed.count("osd_memory_target") || changed.count("osd_memory_base") || changed.count("osd_memory_cache_min") || changed.count("osd_memory_expected_fragmentation")) {
...
void BlueStore::_set_compression() {
    ...
    dout(10) << __func__ << " mode " << Compressor::get_comp_mode_name(comp_mode) << " alg " << (compressor ? compressor->get_type_name() : "(none)") << " min_blob " << comp_min_blob_size
...
void BlueStore::_update_osd_memory_options() {
    ...
    config_changed++;
    dout(10) << __func__ << " osd_memory_target " << osd_memory_target << " osd_memory_base " << osd_memory_base << " osd_memory_expected_fragmentation " << osd_memory_expected_fragmentation
             << " osd_memory_cache_min " << osd_memory_cache_min << dendl;
}
...
int BlueStore::write_meta(const std::string& key, const std::string& value) {
    ...
    if (r < 0) {
        LOG(CEPH_WARN, "_read_bdev_label failed, try objectstore::write_meta(%s,%s)", key.c_str(), value.c_str());
        r = ObjectStore::write_meta(key, value);
        LOG_CHECK_ERR(r);
        return r;
    }
...
int BlueStore::read_meta(const std::string& key, std::string* value) {
    ...
    int r = _read_bdev_label(cct, p, &label);
    if (r < 0) {
        LOG(CEPH_WARN, "_read_bdev_label failed, try objectstore::read_meta(%s)", key.c_str());
        r = ObjectStore::read_meta(key, value);
        LOG_CHECK_ERR(r);
        return r;
    }
    ...
    if (i == label.meta.end()) {
        LOG(CEPH_WARN, "read_meta failed, key %s not found", key.c_str());
        r = ObjectStore::read_meta(key, value);
        LOG_CHECK_ERR(r);
        return r;
    }
...
void BlueStore::_init_logger() {
    ...
    b.add_time_avg(l_bluestore_kv_flush_lat, "kv_flush_lat", "Average kv_thread flush latency", "fl_l", PerfCountersBuilder::PRIO_INTERESTING);
    b.add_time_avg(l_bluestore_kv_commit_lat, "kv_commit_lat", "Average kv_thread commit latency");
    b.add_time_avg(l_bluestore_kv_sync_lat, "kv_sync_lat", "Average kv_sync thread latency", "ks_l", PerfCountersBuilder::PRIO_INTERESTING);
    ...
    b.add_time_avg(l_bluestore_state_aio_wait_lat, "state_aio_wait_lat", "Average aio_wait state latency", "io_l", PerfCountersBuilder::PRIO_INTERESTING);
    ...
    b.add_time_avg(l_bluestore_state_deferred_queued_lat, "state_deferred_queued_lat", "Average deferred_queued state latency");
    b.add_time_avg(l_bluestore_state_deferred_aio_wait_lat, "state_deferred_aio_wait_lat", "Average aio_wait state latency");
    b.add_time_avg(l_bluestore_state_deferred_cleanup_lat, "state_deferred_cleanup_lat", "Average cleanup state latency");
    ...
    b.add_time_avg(l_bluestore_throttle_lat, "throttle_lat", "Average submit throttle latency", "th_l", PerfCountersBuilder::PRIO_CRITICAL);
    b.add_time_avg(l_bluestore_submit_lat, "submit_lat", "Average submit latency", "s_l", PerfCountersBuilder::PRIO_CRITICAL);
    b.add_time_avg(l_bluestore_commit_lat, "commit_lat", "Average commit latency", "c_l", PerfCountersBuilder::PRIO_CRITICAL);
    ...
    b.add_u64_counter(l_bluestore_compress_rejected_count, "compress_rejected_count", "Sum for compress ops rejected due to low net gain of space");
    b.add_u64_counter(l_bluestore_write_pad_bytes, "write_pad_bytes", "Sum for write-op padded bytes", NULL, 0, unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_deferred_write_ops, "deferred_write_ops", "Sum for deferred write op");
    b.add_u64_counter(l_bluestore_deferred_write_bytes, "deferred_write_bytes", "Sum for deferred write bytes", "def", 0, unit_t(UNIT_BYTES));
    ...
    b.add_u64(l_bluestore_compressed, "bluestore_compressed", "Sum for stored compressed bytes", "c", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
    b.add_u64(l_bluestore_compressed_allocated, "bluestore_compressed_allocated", "Sum for bytes allocated for compressed data", "c_a", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
    b.add_u64(l_bluestore_compressed_original, "bluestore_compressed_original", "Sum for original bytes that were compressed", "c_o", PerfCountersBuilder::PRIO_USEFUL, unit_t(UNIT_BYTES));
    ...
    b.add_u64_counter(l_bluestore_onode_hits, "onode_hits", "Count of onode cache lookup hits", "o_ht", PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_onode_misses, "onode_misses", "Count of onode cache lookup misses", "o_ms", PerfCountersBuilder::PRIO_USEFUL);
    b.add_u64_counter(l_bluestore_onode_shard_hits, "onode_shard_hits", "Sum for onode-shard lookups hit in the cache");
    b.add_u64_counter(l_bluestore_onode_shard_misses, "bluestore_onode_shard_misses", "Sum for onode-shard lookups missed in the cache");
    ...
    b.add_u64(l_bluestore_buffer_bytes, "bluestore_buffer_bytes", "Number of buffer bytes in cache", NULL, 0, unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_buffer_hit_bytes, "bluestore_buffer_hit_bytes", "Sum for bytes of read hit in the cache", NULL, 0, unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_buffer_miss_bytes, "bluestore_buffer_miss_bytes", "Sum for bytes of read missed in the cache", NULL, 0, unit_t(UNIT_BYTES));

    b.add_u64_counter(l_bluestore_write_big, "bluestore_write_big", "Large aligned writes into fresh blobs");
    b.add_u64_counter(l_bluestore_write_big_bytes, "bluestore_write_big_bytes", "Large aligned writes into fresh blobs (bytes)", NULL, 0, unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_write_big_blobs, "bluestore_write_big_blobs", "Large aligned writes into fresh blobs (blobs)");
    b.add_u64_counter(l_bluestore_write_big_deferred, "bluestore_write_big_deferred", "Big overwrites using deferred");
    b.add_u64_counter(l_bluestore_write_small, "bluestore_write_small", "Small writes into existing or sparse small blobs");
    b.add_u64_counter(l_bluestore_write_small_bytes, "bluestore_write_small_bytes", "Small writes into existing or sparse small blobs (bytes)", NULL, 0, unit_t(UNIT_BYTES));
    b.add_u64_counter(l_bluestore_write_small_unused, "bluestore_write_small_unused", "Small writes into unused portion of existing blob");
    b.add_u64_counter(l_bluestore_write_deferred, "bluestore_write_deferred", "Total deferred writes submitted");
    b.add_u64_counter(l_bluestore_write_deferred_bytes, "bluestore_write_deferred_bytes", "Total bytes submitted as deferred writes");
    b.add_u64_counter(l_bluestore_write_small_pre_read, "bluestore_write_small_pre_read",
    ...
    b.add_u64_counter(l_bluestore_extent_compress, "bluestore_extent_compress", "Sum for extents that have been removed due to compression");
    ...
    b.add_u64_counter(l_bluestore_reads_with_retries, "bluestore_reads_with_retries", "Read operations that required at least one retry due to failed checksum validation");
    b.add_u64(l_bluestore_fragmentation, "bluestore_fragmentation_micros", "How fragmented bluestore free space is (free extents / max possible number of free extents) * 1000");
    b.add_time_avg(l_bluestore_omap_seek_to_first_lat, "omap_seek_to_first_lat", "Average omap iterator seek_to_first call latency");
    b.add_time_avg(l_bluestore_omap_upper_bound_lat, "omap_upper_bound_lat", "Average omap iterator upper_bound call latency");
    b.add_time_avg(l_bluestore_omap_lower_bound_lat, "omap_lower_bound_lat", "Average omap iterator lower_bound call latency");
...
int BlueStore::_read_bdev_label(CephContext* cct, string path, bluestore_bdev_label_t* label) {
    ...
    if (fd < 0) {
        fd = -errno;
        LOG_ROOT_ERR(fd, "failed to open %s: %s", path.c_str(), cpp_strerror(fd));
        return fd;
    }
    ...
    try {
        ...
        LOG_ROOT_ERR(-ENOENT, "failed to decode %s: %s", path.c_str(), e.what());
        return -ENOENT;
    }
    if (crc != expected_crc) {
        LOG_ROOT_ERR(-EIO, "failed to decode %s: crc mismatch %x != %x", path.c_str(), crc, expected_crc);
        return -EIO;
    }
...
void BlueStore::_set_alloc_sizes(void) {
    ...
    dout(10) << __func__ << " min_alloc_size 0x" << std::hex << min_alloc_size << std::dec << " order " << (int)min_alloc_size_order << " max_alloc_size 0x" << std::hex << max_alloc_size
             << " prefer_deferred_size 0x" << prefer_deferred_size << std::dec << " deferred_batch_ops " << deferred_batch_ops << dendl;
}
...
int BlueStore::_open_fm(KeyValueDB::Transaction t, bool read_only) {
    ...
    if (t) {
        ...
        r = fm->init(db, read_only, [&](const std::string& key, std::string* result) { return read_meta(key, result); });
        if (r < 0) {
    ...
    if (fm->get_size() >= bdev->get_size() + min_alloc_size) {
        ...
        ss << "slow device size mismatch detected, "
           << " fm size(" << fm->get_size() << ") > slow device size(" << bdev->get_size() << "), Please stop using this OSD as it might cause data loss.";
...
int BlueStore::_init_alloc() {
    ...
    dout(1) << __func__ << " loaded " << byte_u_t(bytes) << " in " << num << " extents" << std::hex << ", allocator type " << shared_alloc.a->get_type() << ", capacity 0x"
            << shared_alloc.a->get_capacity() << ", block size 0x" << shared_alloc.a->get_block_size() << ", free 0x" << shared_alloc.a->get_free() << ", fragmentation "
            << shared_alloc.a->get_fragmentation() << std::dec << dendl;
...
int BlueStore::_minimal_open_bluefs(bool create) {
    ...
    bfn = path + "/block.wal";
    if (::stat(bfn.c_str(), &st) == 0) {
        r = bluefs->add_block_device(BlueFS::BDEV_WAL, bfn, create && cct->_conf->bdev_enable_discard, BDEV_LABEL_BLOCK_SIZE);
        if (r < 0) {
            derr << __func__ << " add block device(" << bfn << ") returned: " << cpp_strerror(r) << dendl;
            goto free_bluefs;
        }
...
int BlueStore::_open_bluefs(bool create, bool read_only) {
    ...
    if (bluefs_layout.shared_bdev == BlueFS::BDEV_SLOW) {
        ...
        if (cct->_conf->bluestore_volume_selection_policy == "fit_to_fast") {
            vselector = new FitToFastVolumeSelector(bluefs->get_block_device_size(BlueFS::BDEV_WAL) * 95 / 100, bluefs->get_block_device_size(BlueFS::BDEV_DB) * 95 / 100,
            ...
            vselector = new RocksDBBlueFSVolumeSelector(bluefs->get_block_device_size(BlueFS::BDEV_WAL) * 95 / 100, bluefs->get_block_device_size(BlueFS::BDEV_DB) * 95 / 100,
                                                        bluefs->get_block_device_size(BlueFS::BDEV_SLOW) * 95 / 100,
                                                        1024 * 1024 * 1024,  // FIXME: set expected l0 size here
                                                        rocks_opts.max_bytes_for_level_base, rocks_opts.max_bytes_for_level_multiplier, reserved_factor,
                                                        cct->_conf->bluestore_volume_selection_reserved, cct->_conf->bluestore_volume_selection_policy == "use_some_extra");
        }
    }
    if (create) {
        ...
        *ret = cct->_conf->bluestore_bluefs;
        ...
        if (s == "1") {
            ...
            *ret = false;
            ...
            LOG_ROOT_ERR(-EIO, "BlueStore: bluefs is not 0 or 1, but %s", s.c_str());
            return -EIO;
        }
    }
...
int BlueStore::_open_db_and_around(bool read_only, bool to_repair) {
    dout(0) << __func__ << " read-only:" << read_only << " repair:" << to_repair << dendl;
    {
        ...
        if (type != "bluestore") {
            LOG_ROOT_ERR(-EIO, "BlueStore: type is not bluestore, but %s", type.c_str());
            return -EIO;
        }
    }
...
int BlueStore::_open_collections() {
    ...
    for (it->upper_bound(string()); it->valid(); it->next()) {
        coll_t cid;
        if (cid.parse(it->key())) {
            auto c = ceph::make_ref<Collection>(this, onode_cache_shards[cid.hash_to_shard(onode_cache_shards.size())], buffer_cache_shards[cid.hash_to_shard(buffer_cache_shards.size())], cid);
...
int BlueStore::mkfs() {
    ...
    r = _setup_block_symlink_or_file("block", cct->_conf->bluestore_block_path, cct->_conf->bluestore_block_size, cct->_conf->bluestore_block_create);
    if (r < 0) goto out_close_fsid;
    if (cct->_conf->bluestore_bluefs) {
        r = _setup_block_symlink_or_file("block.wal", cct->_conf->bluestore_block_wal_path, cct->_conf->bluestore_block_wal_size, cct->_conf->bluestore_block_wal_create);
        if (r < 0) goto out_close_fsid;
        r = _setup_block_symlink_or_file("block.db", cct->_conf->bluestore_block_db_path, cct->_conf->bluestore_block_db_size, cct->_conf->bluestore_block_db_create);
        if (r < 0) goto out_close_fsid;
    }
...
int BlueStore::migrate_to_new_bluefs_device(const set<int>& devs_source, int id, const string& dev_path) {
    ...
    if (id == BlueFS::BDEV_NEWWAL) {
        ...
        r = bluefs->add_block_device(BlueFS::BDEV_NEWWAL, dev_path, cct->_conf->bdev_enable_discard, BDEV_LABEL_BLOCK_SIZE);
        ceph_assert(r == 0);

        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWWAL)) {
            r = _check_or_set_bdev_label(dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWWAL), "bluefs wal", true);
            ceph_assert(r == 0);
        }
    } else if (id == BlueFS::BDEV_NEWDB) {
        ...
        if (bluefs->bdev_support_label(BlueFS::BDEV_NEWDB)) {
            r = _check_or_set_bdev_label(dev_path, bluefs->get_block_device_size(BlueFS::BDEV_NEWDB), "bluefs db", true);
            ceph_assert(r == 0);
        }
    }
...
int BlueStore::expand_devices(ostream& out) {
    ...
    for (auto devid : {BlueFS::BDEV_WAL, BlueFS::BDEV_DB}) {
        ...
        out << devid << " : expanding " << " to 0x" << size << std::dec << std::endl;
    ...
    if (size0 < size) {
        out << bluefs_layout.shared_bdev << " : expanding " << " from 0x" << std::hex << size0 << " to 0x" << size << std::dec << std::endl;
...
int BlueStore::_mount() {
    ...
    if ((!per_pool_stat_collection || per_pool_omap != OMAP_PER_PG) && cct->_conf->bluestore_fsck_quick_fix_on_mount == true) {
...
int BlueStore::_fsck_check_extents(std::string_view ctx_descr, const PExtentVector& extents, bool compressed, mempool_dynamic_bitset& used_blocks, uint64_t granularity, BlueStoreRepairer* repairer,
    ...
    for (auto e : extents) {
        ...
        if (depth != FSCK_SHALLOW) {
            bool already = false;
            apply_for_bitset_range(e.offset, e.length, granularity, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
                if (bs.test(pos)) {
                    if (repairer) {
                        repairer->note_misreference(pos * min_alloc_size, min_alloc_size, !already);
                    }
                    if (!already) {
                        derr << __func__ << "::fsck error: " << ctx_descr << ", extent " << e << " or a subset is already allocated (misreferenced)" << dendl;
                        ++errors;
                        already = true;
                    }
                } else
...
void BlueStore::_fsck_check_pool_statfs(BlueStore::per_pool_statfs& expected_pool_statfs, int64_t& errors, int64_t& warnings, BlueStoreRepairer* repairer) {
...
void BlueStore::_fsck_repair_shared_blobs(BlueStoreRepairer& repairer, shared_blob_2hash_tracker_t& sb_ref_counts, sb_info_space_efficient_map_t& sb_info) {
    ...
    auto foreach_shared_blob = [&](std::function<void(coll_t, ghobject_t, uint64_t, const bluestore_blob_t&)> cb) {
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (!c || oid.shard_id != pgid.shard || oid.hobj.get_logical_pool() != (int64_t)pgid.pool() || !c->contains(oid)) {
                    c = nullptr;
                    for (auto& p : coll_map) {
    ...
    foreach_shared_blob([&](coll_t cid, ghobject_t oid, uint64_t sbid, const bluestore_blob_t& b) {
        ...
        for (auto& p : b.get_extents()) {
            if (p.is_valid() && !sb_ref_counts.test_all_zero_range(sbid, p.offset, p.length)) {
                ...
                dout(20) << __func__ << " broken shared blob found for col:" << cid << " obj:" << oid << " sbid 0x " << std::hex << sbid << std::dec << dendl;
                break;
            }
        }
    });
    ...
    while (ref_it != refs_map.end()) {
        ...
        for (cnt = 0; cnt < max_transactions && ref_it != refs_map.end(); ref_it++) {
            ...
            dout(20) << __func__ << " repaired shared_blob 0x" << std::hex << sbid << std::dec << ref_it->second << dendl;
...
BlueStore::OnodeRef BlueStore::fsck_check_objects_shallow(BlueStore::FSCKDepth depth, int64_t pool_id, BlueStore::CollectionRef c, const ghobject_t& oid, const string& key, const bufferlist& value,
                                                          mempool::bluestore_fsck::list<string>* expecting_shards, map<BlobRef, bluestore_blob_t::unused_t>* referenced,
    ...
    for (auto& i : ref_map) {
        ...
        if (blob.is_shared()) {
            ...
            errors += _fsck_check_extents(ctx_descr, blob.get_extents(), blob.is_compressed(), *used_blocks, fm->get_alloc_size(), repairer, *res_statfs, depth);
        } else {
...
class ShallowFSCKThreadPool : public ThreadPool {
   public:
    ...
    struct FSCKWorkQueue : public ThreadPool::WorkQueue_ {
        ...
        FSCKWorkQueue(std::string n, size_t _batchCount, BlueStore* _store, ceph::mutex* _sb_info_lock, sb_info_space_efficient_map_t& _sb_info, shared_blob_2hash_tracker_t& _sb_ref_counts,
        ...
        void _void_process(void* item, TPHandle& handle) override {
            ...
            BlueStore::FSCK_ObjectCtx ctx(batch->errors, batch->warnings, batch->num_objects, batch->num_extents, batch->num_blobs, batch->num_sharded_objects, batch->num_spanning_blobs,
                                          nullptr,  // used_blocks
                                          nullptr,  // used_omap_head
                                          sb_info_lock, *sb_info, *sb_ref_counts, batch->expected_store_statfs, batch->expected_pool_statfs, repairer);

            for (size_t i = 0; i < batch->entry_count; i++) {
                auto& entry = batch->entries[i];

                store->fsck_check_objects_shallow(BlueStore::FSCK_SHALLOW, entry.pool_id, entry.c, entry.oid, entry.key, entry.value,
        ...
        bool queue(int64_t pool_id, BlueStore::CollectionRef c, const ghobject_t& oid, const string& key, const bufferlist& value) {
            bool res = false;
...
void BlueStore::_fsck_check_object_omap(FSCKDepth depth, OnodeRef& o, const BlueStore::FSCK_ObjectCtx& ctx) {
    ...
    if (!o->onode.is_perpool_omap() && !o->onode.is_pgmeta_omap()) {
        if (per_pool_omap == OMAP_PER_POOL) {
            fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: " << o->oid << " has omap that is not per-pool or pgmeta" << fsck_dendl;
            ++errors;
        } else {
            const char* w;
            int64_t num;
            ...
            fsck_derr(num, MAX_FSCK_ERROR_LINES) << "fsck " << w << ": " << o->oid << " has omap that is not per-pool or pgmeta" << fsck_dendl;
        }
    } else if (!o->onode.is_perpg_omap() && !o->onode.is_pgmeta_omap()) {
        if (per_pool_omap == OMAP_PER_PG) {
            fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: " << o->oid << " has omap that is not per-pg or pgmeta" << fsck_dendl;
            ++errors;
        } else {
            const char* w;
            int64_t num;
            ...
            fsck_derr(num, MAX_FSCK_ERROR_LINES) << "fsck " << w << ": " << o->oid << " has omap that is not per-pg or pgmeta" << fsck_dendl;
        }
    }
    if (repairer && !o->onode.is_perpg_omap() && !o->onode.is_pgmeta_omap()) {
        ...
        map<string, bufferlist> kv;
        {
            ...
            uint8_t new_flags = o->onode.flags | bluestore_onode_t::FLAG_PERPOOL_OMAP | bluestore_onode_t::FLAG_PERPG_OMAP;
...
void BlueStore::_fsck_check_objects(FSCKDepth depth, BlueStore::FSCK_ObjectCtx& ctx) {
    ...
    if (it) {
        ...
        std::unique_ptr<WQ> wq(new WQ("FSCKWorkQueue", (thread_count ?: 1) * 32, this, sb_info_lock, sb_info, sb_ref_counts, repairer));
        ...
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (!queued) {
                ...
                o = fsck_check_objects_shallow(depth, pool_id, c, oid, it->key(), it->value(), &expecting_shards, &referenced, ctx);
            }
                        ...
                        for (unsigned p = 0; p < csum_count; ++p) {
                            ...
                            if ((blob.unused & mask) == mask) {
                                ...
                                if (blob.get_csum_item(p) != 0) {
                                    derr << "fsck error: " << oid << " blob claims csum chunk 0x" << std::hex << pos << "~" << csum_chunk_size << " is unused (mask 0x" << mask << " of unused 0x"
                                         << blob.unused << ") but csum is non-zero 0x" << blob.get_csum_item(p) << std::dec << " on blob " << *i.first << dendl;
                                    ++errors;
                                }
                            }
                        }
        ...
        if (depth == FSCK_SHALLOW && thread_count > 0) {
            ...
            if (processed_myself) {
                ...
                dout(0) << __func__ << " partial offload"
                        << ", done myself " << processed_myself << " of " << ctx.num_objects << "objects, threads " << thread_count << dendl;
            }
        }
    }  // if (it)
}
...
/*
 * 当前在fsck中实现的修复逻辑概述，分为两个阶段：检测（+准备）和提交。
 * 检测阶段（按处理顺序）：
 *  (问题 -> 计划执行的修复动作)
 *  - 检测共享Blob的不可解码键 -> 移除
 *  - 检测共享Blob的不可解码记录 -> 移除
 *    （可能触发下方遗漏共享Blob的检测）
 *  - 检测共享Blob的孤立记录 -> 移除
 *  - 检测错误引用的物理扩展块(pextents) -> 修复
 *    准备Bloom过滤器以跟踪cid/oid -> pextent映射
 *    准备错误引用的物理扩展块列表
 *    遍历可能使用'错误引用'物理扩展块的Onode记录
 *    （应用Bloom过滤器以减少计算量）
 *      对每个可疑Onode枚举所有Blob并识别损坏的Blob
 *      （即存在'错误引用'的Blob）
 *      通过分配新物理扩展块并复制数据来重写损坏的Blob数据
 *      若Blob是共享的 -> 取消共享并标记对应共享Blob待删除
 *      释放之前分配的空间
 *      更新扩展映射表(Extent Map)
 *  - 检测遗漏的共享Blob -> 重新创建
 *  - 检测不可解码的延迟事务 -> 移除
 *  - 检测Freelist Manager的'虚假空闲'条目 -> 标记为已用
 *  - 检测Freelist Manager的泄漏条目 -> 标记为空闲
 *  - 检测StatFS不一致性 -> 更新
 * 提交阶段（每个步骤独立提交到数据库）：
 *  - 应用Freelist Manager泄漏条目的修复
 *  - 应用'虚假空闲'条目的修复
 *  - 应用'移除'操作
 *  - 应用物理扩展块引用错误的修复
 *  - 应用共享Blob重建
...
int BlueStore::_fsck(BlueStore::FSCKDepth depth, bool repair) {
    ...
    // 如果我们指定了repair，那么read_only就是false
    bool read_only = !(repair || depth == FSCK_DEEP);

    LOG(CEPH_INFO, "开始调用_open_db_and_around(read_only=%d, to_repair=false)", read_only);

    // int _open_db_and_around(bool read_only, bool to_repair = false);
    int r = _open_db_and_around(read_only /*false*/, false /*to_repair*/);
    LOG_CHECK_ERR_RETURN(r);
...
int BlueStore::_fsck_on_open(BlueStore::FSCKDepth depth, bool repair) {
    uint64_t sb_hash_size = uint64_t(cct->_conf.get_val<Option::size_t>("osd_memory_target") * cct->_conf.get_val<double>("bluestore_fsck_shared_blob_tracker_size"));
    ...
    apply_for_bitset_range(0, std::max<uint64_t>(min_alloc_size, SUPER_RESERVED), alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) { bs.set(pos); });
    ...
    // walk PREFIX_OBJ
    {
        ...
        BlueStore::FSCK_ObjectCtx ctx(errors, warnings, num_objects, num_extents, num_blobs, num_sharded_objects, num_spanning_blobs, &used_blocks, &used_omap_head,
                                      // no need for the below lock when in non-shallow mode as
                                      //  there is no multithreading in this case
                                      depth == FSCK_SHALLOW ? &sb_info_lock : nullptr, sb_info, sb_ref_counts, expected_store_statfs, expected_pool_statfs, repair ? &repairer : nullptr);
    ...
    if (sb_ref_mismatches != 0) {
        derr << "fsck error: shared blob references aren't matching, at least " << sb_ref_mismatches << " found" << dendl;
        ...
        for (it->lower_bound(string()); it->valid(); it->next()) {
            ...
            if (p == sb_info.end()) {
                if (sb_ref_mismatches > 0) {
                    ...
                    dout(5) << __func__ << " found duplicate(?) stray shared blob data for sbid 0x" << std::hex << sbid << std::dec << dendl;
                } else {
                ...
                try {
                    ...
                    if (repair) {
                        dout(20) << __func__ << " undecodable Shared Blob, key:'" << pretty_binary_string(key) << "', removing" << dendl;
                        repairer.remove_key(db, PREFIX_SHARED_BLOB, key);
                    }
                    continue;
                }
                ...
                errors += _fsck_check_extents(ss.str(), extents, sbi.allocated_chunks < 0, used_blocks, fm->get_alloc_size(), repair ? &repairer : nullptr, *expected_statfs, depth);
            }
        }
                ...
                for (auto b : blobs) {
                    ...
                    for (auto e = pextents.begin(); e != pextents.end(); e++) {
                        ...
                        if (alloc_len < 0 || alloc_len < (int64_t)e->length) {
                            derr << __func__ << " failed to allocate 0x" << std::hex << e->length << " allocated 0x " << (alloc_len < 0 ? 0 : alloc_len) << " min_alloc_size 0x" << min_alloc_size
    ...
    if (depth != FSCK_SHALLOW) {
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: found stray omap data on omap_head " << omap_head << " " << last_omap_head << " " << used_omap_head.count(omap_head)
                                                            << fsck_dendl;
                    ++errors;
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                _key_decode_u64(it->key().c_str(), &omap_head);
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: found stray (pgmeta) omap data on omap_head " << omap_head << " " << last_omap_head << " "
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: found stray (per-pool) omap data on omap_head " << omap_head << " " << last_omap_head << " "
                                                            << used_omap_head.count(omap_head) << fsck_dendl;
                    ++errors;
                    last_omap_head = omap_head;
                }
        ...
        if (it) {
            ...
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                c = _key_decode_u64(c, &omap_head);
                if (used_omap_head.count(omap_head) == 0 && omap_head != last_omap_head) {
                    fsck_derr(errors, MAX_FSCK_ERROR_LINES) << "fsck error: found stray (per-pg) omap data on omap_head " << omap_head << " " << last_omap_head << " "
                                                            << used_omap_head.count(omap_head) << fsck_dendl;
                    ++errors;
                    last_omap_head = omap_head;
                }
        ...
        if (it) {
            for (it->lower_bound(string()); it->valid(); it->next()) {
                ...
                for (auto e = wt.released.begin(); e != wt.released.end(); ++e) {
                    apply_for_bitset_range(e.get_start(), e.get_len(), alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) { bs.set(pos); });
                }
            }
        }
        ...
        dout(1) << __func__ << " checking freelist vs allocated" << dendl;
        {
            ...
            while (fm->enumerate_next(db, &offset, &length)) {
                ...
                apply_for_bitset_range(offset, length, alloc_size, used_blocks, [&](uint64_t pos, mempool_dynamic_bitset& bs) {
                    ceph_assert(pos < bs.size());
                    if (bs.test(pos) && !bluefs_used_blocks.test(pos)) {
                        if (offset == SUPER_RESERVED && length == min_alloc_size - SUPER_RESERVED) {
                            ...
                            dout(10) << __func__ << " ignoring free extent between SUPER_RESERVED"
                                     << " and min_alloc_size, 0x" << std::hex << offset << "~" << length << std::dec << dendl;
                        } else {
                            intersects = true;
                            if (repair) {
...
out_scan:
    ...
    dout(1) << __func__ << " <<<FINISH>>> with " << errors << " errors, " << warnings << " warnings, " << repaired << " repaired, " << (errors + warnings - (int)repaired) << " remaining in "
...
void BlueStore::inject_legacy_omap(coll_t cid, ghobject_t oid) {
    ...
    o->onode.clear_flag(bluestore_onode_t::FLAG_PERPG_OMAP | bluestore_onode_t::FLAG_PERPOOL_OMAP | bluestore_onode_t::FLAG_PGMETA_OMAP);
    txn = db->get_transaction();
    _record_onode(o, txn);
    db->submit_transaction_sync(txn);
}
...
void BlueStore::_get_statfs_overall(struct store_statfs_t* buf) {
    ...
    auto prefix = per_pool_omap == OMAP_BULK ? PREFIX_OMAP : per_pool_omap == OMAP_PER_POOL ? PREFIX_PERPOOL_OMAP : PREFIX_PERPG_OMAP;
...
ObjectStore::CollectionHandle BlueStore::open_collection(const coll_t& cid) { return _get_collection(cid); }
...
int BlueStore::read(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length, bufferlist& bl, uint32_t op_flags) {
    ...
    int r;
    {
        ...
        auto start1 = mono_clock::now();
        OnodeRef o = c->get_onode(oid, false);
        log_latency("get_onode@read", l_bluestore_read_onode_meta_lat, mono_clock::now() - start1, cct->_conf->bluestore_log_op_age);
        if (!o || !o->exists) {
            r = -ENOENT;
            goto out;
        }
...
out:
    if (r >= 0 && _debug_data_eio(oid)) {
        ...
        derr << __func__ << " " << c->cid << " " << oid << " INJECT EIO" << dendl;
               ...
               cct->_conf->bluestore_debug_random_read_err && (rand() % (int)(cct->_conf->bluestore_debug_random_read_err * 100.0)) == 0) {
...
void BlueStore::_read_cache(OnodeRef& o, uint64_t offset, size_t length, int read_cache_policy, ready_regions_t& ready_regions, blobs2read_t& blobs2read) {
    ...
    while (left > 0 && lp != o->extent_map.extent_map.end()) {
        ...
        bptr->shared_blob->bc.read(bptr->shared_blob->get_cache(), b_off, b_len, cache_res, cache_interval, read_cache_policy);
...
int BlueStore::_prepare_read_ioc(blobs2read_t& blobs2read, vector<bufferlist>* compressed_blob_bls, IOContext* ioc) {
    for (auto& p : blobs2read) {
        ...
        if (bptr->get_blob().is_compressed()) {
            ...
            auto r = bptr->get_blob().map(0, bptr->get_blob().get_ondisk_length(), [&](uint64_t offset, uint64_t length) {
            ...
            // read the pieces
            for (auto& req : r2r) {
                dout(20) << __func__ << "    region 0x" << std::hex << req.regs.front().logical_offset << ": 0x" << req.regs.front().blob_xoffset << " reading 0x" << req.r_off << "~" << req.r_len
                         << std::dec << dendl;
...
int BlueStore::_generate_read_result_bl(OnodeRef& o, uint64_t offset, size_t length, ready_regions_t& ready_regions, vector<bufferlist>& compressed_blob_bls, blobs2read_t& blobs2read, bool buffered,
...
int BlueStore::_do_read(Collection* c, OnodeRef& o, uint64_t offset, size_t length, bufferlist& bl, uint32_t op_flags, uint64_t retry_count) {
    FUNCTRACE(cct);
    int r = 0;
    ...
    if (op_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
        ...
        buffered = true;
    } else if (cct->_conf->bluestore_default_buffered_read && (op_flags & (CEPH_OSD_OP_FLAG_FADVISE_DONTNEED | CEPH_OSD_OP_FLAG_FADVISE_NOCACHE)) == 0) {
    ...
    log_latency_fn(__func__, l_bluestore_read_wait_aio_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age, [&](auto lat) { return ", num_ios = " + stringify(num_ios); });

    bool csum_error = false;
    r = _generate_read_result_bl(o, offset, length, ready_regions, compressed_blob_bls, blobs2read, buffered, &csum_error, bl);
    if (csum_error) {
...
int BlueStore::_verify_csum(OnodeRef& o, const bluestore_blob_t* blob, uint64_t blob_xoffset, const bufferlist& bl, uint64_t logical_offset) const {
    int bad;
    uint64_t bad_csum;
    auto start = mono_clock::now();
    int r = blob->verify_csum(blob_xoffset, bl, &bad, &bad_csum);
    if (cct->_conf->bluestore_debug_inject_csum_err_probability > 0 && (rand() % 10000) < cct->_conf->bluestore_debug_inject_csum_err_probability * 10000.0) {
    ...
    if (r < 0) {
        if (r == -1) {
            ...
            derr << __func__ << " bad " << Checksummer::get_csum_type_string(blob->csum_type) << "/0x" << std::hex << blob->get_csum_chunk_size() << " checksum at blob offset 0x" << bad << ", got 0x"
                 << bad_csum << ", expected 0x" << blob->get_csum_item(bad / blob->get_csum_chunk_size()) << std::dec << ", device location " << pex << ", logical extent 0x" << std::hex
                 << (logical_offset + bad - blob_xoffset) << "~" << blob->get_csum_chunk_size() << std::dec << ", object " << o->oid << dendl;
        } else {
...
// use it internally
int BlueStore::_fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length, interval_set<uint64_t>& destset) {
    Collection* c = static_cast<Collection*>(c_.get());
    if (!c->exists) return -ENOENT;
    {
        std::shared_lock l(c->lock);
...
int BlueStore::fiemap(CollectionHandle& c_, const ghobject_t& oid, uint64_t offset, size_t length, map<uint64_t, uint64_t>& destmap) {
    interval_set<uint64_t> m;
    int r = _fiemap(c_, oid, offset, length, m);
    if (r >= 0) {
...
int BlueStore::readv(CollectionHandle& c_, const ghobject_t& oid, interval_set<uint64_t>& m, bufferlist& bl, uint32_t op_flags) {
    auto start = mono_clock::now();
    Collection* c = static_cast<Collection*>(c_.get());
    const coll_t& cid = c->get_cid();
...
int BlueStore::_do_readv(Collection* c, OnodeRef& o, const interval_set<uint64_t>& m, bufferlist& bl, uint32_t op_flags, uint64_t retry_count) {
    FUNCTRACE(cct);
    int r = 0;
    int read_cache_policy = 0;  // do not bypass clean or dirty cache
    ...
    for (auto p = m.begin(); p != m.end(); p++, i++) {
        ...
        _read_cache(o, p.get_start(), p.get_len(), read_cache_policy, std::get<0>(raw_results[i]), std::get<2>(raw_results[i]));
    ...
    for (auto p = m.begin(); p != m.end(); p++, i++) {
        bool csum_error = false;
        bufferlist t;
        r = _generate_read_result_bl(o, p.get_start(), p.get_len(), std::get<0>(raw_results[i]), std::get<1>(raw_results[i]), std::get<2>(raw_results[i]), buffered, &csum_error, t);
        if (csum_error) {
            // Handles spurious read errors caused by a kernel bug.
            // We sometimes get all-zero pages as a result of the read under
            // high memory pressure. Retrying the failing read succeeds in most
...
int BlueStore::list_collections(vector<coll_t>& ls) {
    ...
    for (ceph::unordered_map<coll_t, CollectionRef>::iterator p = coll_map.begin(); p != coll_map.end(); ++p) ls.push_back(p->first);
    return 0;
}
...
int BlueStore::collection_list(CollectionHandle& c_, const ghobject_t& start, const ghobject_t& end, int max, vector<ghobject_t>* ls, ghobject_t* pnext) {
    Collection* c = static_cast<Collection*>(c_.get());
    c->flush();
    ...
    dout(10) << __func__ << " " << c->cid << " start " << start << " end " << end << " max " << max << " = " << r << ", ls.size() = " << ls->size() << ", next = " << (pnext ? *pnext : ghobject_t())
             << dendl;
    return r;
}

int BlueStore::collection_list_legacy(CollectionHandle& c_, const ghobject_t& start, const ghobject_t& end, int max, vector<ghobject_t>* ls, ghobject_t* pnext) {
    Collection* c = static_cast<Collection*>(c_.get());
    c->flush();
    dout(15) << __func__ << " " << c->cid << " start " << start << " end " << end << " max " << max << dendl;
    int r;
...
int BlueStore::_collection_list(Collection* c, const ghobject_t& start, const ghobject_t& end, int max, bool legacy, vector<ghobject_t>* ls, ghobject_t* pnext) {
    if (!c->exists) return -ENOENT;
    ...
    auto log_latency = make_scope_guard([&, start_time = mono_clock::now(), func_name = __func__] {
        log_latency_fn(func_name, l_bluestore_remove_lat, mono_clock::now() - start_time, cct->_conf->bluestore_log_collection_list_age, [&](const ceph::timespan& lat) {
            ostringstream ostr;
            ostr << ", lat = " << timespan_str(lat) << " cid =" << c->cid << " start " << start << " end " << end << " max " << max;
            return ostr.str();
        });
    });
    ...
    get_coll_range(c->cid, c->cnode.bits, &coll_range_temp_start, &coll_range_temp_end, &coll_range_start, &coll_range_end, legacy);
...
out:
    c->store->log_latency(__func__, l_bluestore_omap_get_keys_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
out:
    c->store->log_latency(__func__, l_bluestore_omap_get_values_lat, mono_clock::now() - start1, c->store->cct->_conf->bluestore_log_omap_iterator_age);
...
BlueStore::TransContext* BlueStore::_txc_create(Collection* c, OpSequencer* osr, list<Context*>* on_commits, TrackedOpRef osd_op) {
...
int BlueStore::_zoned_check_config_settings() {
    if (cct->_conf->bluestore_allocator != "zoned") {
        dout(1) << __func__ << " The drive is HM-SMR but " << cct->_conf->bluestore_allocator << " allocator is specified. "
...
#if defined(WITH_LTTNG)
        if (txc->tracing) {
            tracepoint(bluestore, transaction_kv_submit_latency, txc->osr->get_sequencer_id(), txc->seq, sync_submit_transaction, ceph::to_seconds<double>(mono_clock::now() - start));
        }
#endif
    }
...
void BlueStore::_txc_committed_kv(TransContext* txc) {
    ...
    log_latency_fn(__func__, l_bluestore_commit_lat, mono_clock::now() - txc->start, cct->_conf->bluestore_log_op_age, [&](auto lat) { return ", txc = " + stringify(txc); });
}
    ...
    OpSequencer::q_list_t releasing_txc;
    {
        ...
        while (!osr->q.empty()) {
            ...
            if (txc->get_state() != TransContext::STATE_DONE) {
                ...
                if (txc->get_state() == TransContext::STATE_DEFERRED_QUEUED && osr->q.size() > g_conf()->bluestore_max_deferred_txc) {
...
void BlueStore::_kv_sync_thread() {
    ...
    while (true) {
        ...
        if (kv_queue.empty() && ((deferred_done_queue.empty() && deferred_stable_queue.empty()) || !deferred_aggressive)) {
            ...
            dout(20) << __func__ << " committing " << kv_queue.size() << " submitting " << kv_queue_unsubmitted.size() << " deferred done " << deferred_done_queue.size() << " stable "
            ...
            for (auto txc : kv_committing) {
                if (txc->tracing) {
                    tracepoint(bluestore, transaction_kv_sync_latency, txc->osr->get_sequencer_id(), txc->seq, kv_committing.size(), deferred_done.size(), deferred_stable.size(), sync_latency);
                }
            }
#endif

            {
                ...
                if (kv_committing_to_finalize.empty()) {
                    ...
                    kv_committing_to_finalize.insert(kv_committing_to_finalize.end(), kv_committing.begin(), kv_committing.end());
                    kv_committing.clear();
                }
                if (deferred_stable_to_finalize.empty()) {
                    deferred_stable_to_finalize.swap(deferred_stable);
                } else {
                    deferred_stable_to_finalize.insert(deferred_stable_to_finalize.end(), deferred_stable.begin(), deferred_stable.end());
...
void BlueStore::_kv_finalize_thread() {
    ...
    while (true) {
        ...
        if (kv_committing_to_finalize.empty() && deferred_stable_to_finalize.empty()) {
            ...
            log_latency("kv_final", l_bluestore_kv_final_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
...
void BlueStore::_zoned_clean_zone(uint64_t zone_num) { dout(10) << __func__ << " cleaning zone " << zone_num << dendl; }
...
bool BlueStore::_eliminate_outdated_deferred(bluestore_deferred_transaction_t* deferred_txn, interval_set<uint64_t>& bluefs_extents) {
    bool has_some = false;
...
int BlueStore::queue_transactions(CollectionHandle& ch, vector<Transaction>& tls, TrackedOpRef op, ThreadPool::TPHandle* handle) {
    FUNCTRACE(cct);
...
void BlueStore::_txc_add_transaction(TransContext* txc, Transaction* t) {
    ...
    for (int pos = 0; i.have_op(); ++pos) {
        ...
        bool create = false;
        if (op->op == Transaction::OP_TOUCH || op->op == Transaction::OP_CREATE || op->op == Transaction::OP_WRITE || op->op == Transaction::OP_ZERO) {
            create = true;
        }
    ...
    endop:
        if (r < 0) {
            ...
            if (r == -ENOENT && !(op->op == Transaction::OP_CLONERANGE || op->op == Transaction::OP_CLONE || op->op == Transaction::OP_CLONERANGE2 || op->op == Transaction::OP_COLL_ADD ||
                                  op->op == Transaction::OP_SETATTR || op->op == Transaction::OP_SETATTRS || op->op == Transaction::OP_RMATTR || op->op == Transaction::OP_OMAP_SETKEYS ||
                                  op->op == Transaction::OP_OMAP_RMKEYS || op->op == Transaction::OP_OMAP_RMKEYRANGE || op->op == Transaction::OP_OMAP_SETHEADER))
            ...
            if (!ok) {
                ...
                if (r == -ENOENT && (op->op == Transaction::OP_CLONERANGE || op->op == Transaction::OP_CLONE || op->op == Transaction::OP_CLONERANGE2)) msg = "ENOENT on clone suggests osd bug";
...
void BlueStore::_do_write_small(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length, bufferlist::iterator& blp, WriteContext* wctx) {
    ...
    do {
        ...
        if (ep != end && ep->logical_offset < offset + max_bsize) {
            ...
            if (bstart >= end_offs) {
                ...
                if ((b_off % chunk_size == 0 && b_len % chunk_size == 0) && b->get_blob().get_ondisk_length() >= b_off + b_len && b->get_blob().is_unused(b_off, b_len) &&
                ...
                if ((head_read || tail_read) && (b->get_blob().get_ondisk_length() >= b_off + b_len + tail_read) && head_read + tail_read < min_alloc_size) {
                    b_off -= head_read;
                ...
                if (b->get_blob().get_ondisk_length() >= b_off + b_len && b_off % chunk_size == 0 && b_len % chunk_size == 0 && b->get_blob().is_allocated(b_off, b_len)) {
                    _apply_padding(head_pad, tail_pad, bl);
...
bool BlueStore::BigDeferredWriteContext::can_defer(BlueStore::extent_map_t::iterator ep, uint64_t prefer_deferred_size, uint64_t block_size, uint64_t offset, uint64_t l) {
    bool res = false;
    ...
    if (offset >= ep->blob_start() && blob.is_mutable()) {
        ...
        res = blob_aligned_len() < prefer_deferred_size && blob_aligned_len() <= ondisk && blob.is_allocated(b_off, blob_aligned_len());
        if (res) {
            blob_ref = ep->blob;
...
bool BlueStore::BigDeferredWriteContext::apply_defer() {
    int r = blob_ref->get_blob().map(b_off, blob_aligned_len(), [&](const bluestore_pextent_t& pext, uint64_t offset, uint64_t length) {
...
void BlueStore::_do_write_big_apply_deferred(TransContext* txc, CollectionRef& c, OnodeRef& o, BlueStore::BigDeferredWriteContext& dctx, bufferlist::iterator& blp, WriteContext* wctx) {
    bufferlist bl;
...
void BlueStore::_do_write_big(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length, bufferlist::iterator& blp, WriteContext* wctx) {
    ...
    while (length > 0) {
        ...
        if (!wctx->compress) {
            ...
            if (prefer_deferred_size_snapshot && l <= prefer_deferred_size_snapshot * 2) {
                ...
                bool will_defer = ep != end ? head_info.can_defer(ep, prefer_deferred_size_snapshot, block_size, offset, l) : false;
                ...
                if (will_defer) {
                    dout(20) << __func__ << " " << *(head_info.blob_ref) << " deferring big " << std::hex << " (0x" << head_info.b_off << "~" << head_info.blob_aligned_len() << ")" << std::dec
                             << " write via deferred" << dendl;
                    if (remaining) {
                        dout(20) << __func__ << " " << *(tail_info.blob_ref) << " deferring big " << std::hex << " (0x" << tail_info.b_off << "~" << tail_info.blob_aligned_len() << ")" << std::dec
                                 << " write via deferred" << dendl;
                    }
            ...
            do {
                any_change = false;
                if (ep != end && ep->logical_offset < offset + max_bsize) {
                    dout(20) << __func__ << " considering " << *ep << " bstart 0x" << std::hex << ep->blob_start() << std::dec << dendl;

                    if (offset >= ep->blob_start() && ep->blob->can_reuse_blob(min_alloc_size, max_bsize, offset - ep->blob_start(), &l)) {
                        b = ep->blob;
                        b_off = offset - ep->blob_start();
                ...
                if (prev_ep != end && prev_ep->logical_offset >= min_off) {
                    dout(20) << __func__ << " considering rev " << *prev_ep << " bstart 0x" << std::hex << prev_ep->blob_start() << std::dec << dendl;
        ...
        dout(20) << __func__ << " schedule write big: 0x" << std::hex << offset << "~" << l << std::dec << (new_blob ? " new " : " reuse ") << *b << dendl;
        offset += l;
        length -= l;
...
int BlueStore::_do_alloc_write(TransContext* txc, CollectionRef coll, OnodeRef& o, WriteContext* wctx) {
    ...
    for (auto& wi : wctx->writes) {
        if (c && wi.blob_length > min_alloc_size) {
            ...
            if (r == 0 && result_len <= want_len && result_len < wi.blob_length) {
                ...
                dout(5) << __func__ << std::hex << "  0x" << wi.blob_length << " bytes compressed using " << c->get_type_name() << std::dec << " failed with errcode = " << r
            ...
            if (rejected) {
                dout(20) << __func__ << std::hex << "  0x" << wi.blob_length << " compressed to 0x" << compressed_len << " -> 0x" << result_len << " with " << c->get_type()
                         << ", which is more than required 0x" << want_len_raw << " -> 0x" << want_len << ", leaving uncompressed" << std::dec << dendl;
                logger->inc(l_bluestore_compress_rejected_count);
                need += wi.blob_length;
                data_size += wi.bl.length();
            }
            log_latency("compress@_do_alloc_write", l_bluestore_compress_lat, mono_clock::now() - start, cct->_conf->bluestore_log_op_age);
        } else {
            need += wi.blob_length;
            data_size += wi.bl.length();
        }
    ...
    if (prealloc_left < 0 || prealloc_left < (int64_t)need) {
        derr << __func__ << " failed to allocate 0x" << std::hex << need << " allocated 0x " << (prealloc_left < 0 ? 0 : prealloc_left) << " min_alloc_size 0x" << min_alloc_size << " available 0x "
    ...
    dout(20) << __func__ << std::hex << " need=0x" << need << " data=0x" << data_size << " prealloc " << prealloc << dendl;
    ...
    for (auto& wi : wctx->writes) {
        ...
        if (wi.compressed) {
            ...
            if (csum != Checksummer::CSUM_NONE) {
                dout(20) << __func__ << " initialize csum setting for compressed blob " << *wi.b << " csum_type " << Checksummer::get_csum_type_string(csum) << " csum_order " << csum_order
                         << " csum_length 0x" << std::hex << csum_length << " blob_length 0x" << wi.blob_length << " compressed_length 0x" << wi.compressed_len << std::dec << dendl;
            ...
            if ((suggested_boff % (1 << csum_order)) == 0 && suggested_boff + final_length <= max_bsize && suggested_boff > b_off) {
            ...
            if (csum != Checksummer::CSUM_NONE) {
                dout(20) << __func__ << " initialize csum setting for new blob " << *wi.b << " csum_type " << Checksummer::get_csum_type_string(csum) << " csum_order " << csum_order
                         << " csum_length 0x" << std::hex << csum_length << std::dec << dendl;
                dblob.init_csum(csum, csum_order, csum_length);
            }
        }
        ...
        if (!g_conf()->bluestore_debug_omit_block_device_write) {
            if (data_size < prefer_deferred_size_snapshot) {
                ...
                wi.b->get_blob().map_bl(b_off, *l, [&](uint64_t offset, bufferlist& t) { bdev->aio_write(offset, t, &txc->ioc, false); });
...
void BlueStore::_wctx_finish(TransContext* txc, CollectionRef& c, OnodeRef& o, WriteContext* wctx, set<SharedBlob*>* maybe_unshared_blobs) {
...
void BlueStore::_do_write_data(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length, bufferlist& bl, WriteContext* wctx) {
    uint64_t end = offset + length;
...
void BlueStore::_choose_write_options(CollectionRef& c, OnodeRef& o, uint32_t fadvise_flags, WriteContext* wctx) {
    if (fadvise_flags & CEPH_OSD_OP_FLAG_FADVISE_WILLNEED) {
        ...
        wctx->buffered = true;
    } else if (cct->_conf->bluestore_default_buffered_write && (fadvise_flags & (CEPH_OSD_OP_FLAG_FADVISE_DONTNEED | CEPH_OSD_OP_FLAG_FADVISE_NOCACHE)) == 0) {
    ...
    wctx->compress = (cm != Compressor::COMP_NONE) && ((cm == Compressor::COMP_FORCE) || (cm == Compressor::COMP_AGGRESSIVE && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_INCOMPRESSIBLE) == 0) ||
                                                       (cm == Compressor::COMP_PASSIVE && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_COMPRESSIBLE)));

    if ((alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_SEQUENTIAL_READ) && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_READ) == 0 &&
        (alloc_hints & (CEPH_OSD_ALLOC_HINT_FLAG_IMMUTABLE | CEPH_OSD_ALLOC_HINT_FLAG_APPEND_ONLY)) && (alloc_hints & CEPH_OSD_ALLOC_HINT_FLAG_RANDOM_WRITE) == 0) {
    ...
    dout(20) << __func__ << " prefer csum_order " << wctx->csum_order << " target_blob_size 0x" << std::hex << wctx->target_blob_size << " compress=" << (int)wctx->compress
             << " buffered=" << (int)wctx->buffered << std::dec << dendl;
}

int BlueStore::_do_gc(TransContext* txc, CollectionRef& c, OnodeRef& o, const WriteContext& wctx, uint64_t* dirty_start, uint64_t* dirty_end) {
...
int BlueStore::_do_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, uint64_t length, bufferlist& bl, uint32_t fadvise_flags) {
    int r = 0;

    dout(20) << __func__ << " " << o->oid << " 0x" << std::hex << offset << "~" << length << " - have 0x" << o->onode.size << " (" << std::dec << o->onode.size << ")"
             << " bytes" << std::hex << " fadvise_flags 0x" << fadvise_flags << " alloc_hint 0x" << o->onode.alloc_hint_flags << " expected_object_size " << o->onode.expected_object_size
...
int BlueStore::_write(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, size_t length, bufferlist& bl, uint32_t fadvise_flags) {
...
void BlueStore::_do_truncate(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t offset, set<SharedBlob*>* maybe_unshared_blobs) {
...
int BlueStore::_do_remove(TransContext* txc, CollectionRef& c, OnodeRef& o) {
    ...
    for (auto& s : o->extent_map.shards) {
        ...
        generate_extent_shard_key_and_apply(o->key, s.shard_info->offset, &key, [&](const string& final_key) { txc->t->rmkey(PREFIX_OBJ, final_key); });
    }
...
int BlueStore::_remove(TransContext* txc, CollectionRef& c, OnodeRef& o) {
    ...
    log_latency_fn(__func__, l_bluestore_remove_lat, mono_clock::now() - start_time, cct->_conf->bluestore_log_op_age, [&](const ceph::timespan& lat) {
        ostringstream ostr;
...
int BlueStore::_omap_rmkey_range(TransContext* txc, CollectionRef& c, OnodeRef& o, const string& first, const string& last) {
    dout(15) << __func__ << " " << c->cid << " " << o->oid << dendl;
...
int BlueStore::_set_alloc_hint(TransContext* txc, CollectionRef& c, OnodeRef& o, uint64_t expected_object_size, uint64_t expected_write_size, uint32_t flags) {
    dout(15) << __func__ << " " << c->cid << " " << o->oid << " object_size " << expected_object_size << " write_size " << expected_write_size << " flags " << ceph_osd_alloc_hint_flag_string(flags)
             << dendl;
    int r = 0;
    ...
    txc->write_onode(o);
    dout(10) << __func__ << " " << c->cid << " " << o->oid << " object_size " << expected_object_size << " write_size " << expected_write_size << " flags " << ceph_osd_alloc_hint_flag_string(flags)
             << " = " << r << dendl;
    return r;
}
...
int BlueStore::_do_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff, uint64_t length, uint64_t dstoff) {
    dout(15) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " 0x" << std::hex << srcoff << "~" << length << " -> "
...
int BlueStore::_clone_range(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, uint64_t srcoff, uint64_t length, uint64_t dstoff) {
...
out:
    dout(10) << __func__ << " " << c->cid << " " << oldo->oid << " -> " << newo->oid << " from 0x" << std::hex << srcoff << "~" << length << " to offset 0x" << dstoff << std::dec << " = " << r
             << dendl;
    return r;
}
...
int BlueStore::_rename(TransContext* txc, CollectionRef& c, OnodeRef& oldo, OnodeRef& newo, const ghobject_t& new_oid) {
    ...
    // rewrite shards
    {
        ...
        for (auto& s : oldo->extent_map.shards) {
            generate_extent_shard_key_and_apply(oldo->key, s.shard_info->offset, &key, [&](const string& final_key) { txc->t->rmkey(PREFIX_OBJ, final_key); });
            s.dirty = true;
        }
    }
...
void BlueStore::log_latency(const char* name, int idx, const ceph::timespan& l, double lat_threshold, const char* info) const {
    logger->tinc(idx, l);
...
void BlueStore::log_latency_fn(const char* name, int idx, const ceph::timespan& l, double lat_threshold, std::function<string(const ceph::timespan& lat)> fn) const {
    logger->tinc(idx, l);
    if (lat_threshold > 0.0 && l >= make_timespan(lat_threshold)) {
...
#if defined(WITH_LTTNG)
void BlueStore::BlueStoreThrottle::emit_initial_tracepoint(KeyValueDB& db, TransContext& txc, mono_clock::time_point start_throttle_acquire) {
    ...
    if (should_trace(&started, &completed)) {
        txc.tracing = true;
        uint64_t rocksdb_base_level, rocksdb_estimate_pending_compaction_bytes, rocksdb_cur_size_all_mem_tables, rocksdb_compaction_pending, rocksdb_mem_table_flush_pending,
            rocksdb_num_running_compactions, rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate;
        ...
        tracepoint(bluestore, transaction_initial_state, txc.osr->get_sequencer_id(), txc.seq, throttle_bytes.get_current(), throttle_deferred_bytes.get_current(), pending_kv_ios,
                   pending_deferred_ios, started, completed, ceph::to_seconds<double>(mono_clock::now() - start_throttle_acquire));

        tracepoint(bluestore, transaction_initial_state_rocksdb, txc.osr->get_sequencer_id(), txc.seq, rocksdb_base_level, rocksdb_estimate_pending_compaction_bytes, rocksdb_cur_size_all_mem_tables,
                   rocksdb_compaction_pending, rocksdb_mem_table_flush_pending, rocksdb_num_running_compactions, rocksdb_num_running_flushes, rocksdb_actual_delayed_write_rate);
    }
}
#endif
...
mono_clock::duration BlueStore::BlueStoreThrottle::log_state_latency(TransContext& txc, PerfCounters* logger, int state) {
    ...
    if (txc.tracing && state >= l_bluestore_state_prepare_lat && state <= l_bluestore_state_done_lat) {
        ...
        tracepoint(bluestore, transaction_state_duration, txc.osr->get_sequencer_id(), txc.seq, state, ceph::to_seconds<double>(lat));
    }
#endif
    txc.last_stamp = now;
    return lat;
...
bool BlueStore::BlueStoreThrottle::try_start_transaction(KeyValueDB& db, TransContext& txc, mono_clock::time_point start_throttle_acquire) {
...
void BlueStore::BlueStoreThrottle::finish_start_transaction(KeyValueDB& db, TransContext& txc, mono_clock::time_point start_throttle_acquire) {
...
void BlueStore::BlueStoreThrottle::complete_kv(TransContext& txc) {
    ...
    if (txc.tracing) {
        tracepoint(bluestore, transaction_commit_latency, txc.osr->get_sequencer_id(), txc.seq, ceph::to_seconds<double>(mono_clock::now() - txc.start));
    }
}
#endif
...
void BlueStore::BlueStoreThrottle::complete(TransContext& txc) {
    ...
    if (txc.tracing) {
        ...
        tracepoint(bluestore, transaction_total_duration, txc.osr->get_sequencer_id(), txc.seq, ceph::to_seconds<double>(lat));
    }
}
#endif
...
int BlueStore::DBHistogram::get_value_slab(size_t sz) { return (sz / VALUE_SLAB); }
...
void BlueStore::DBHistogram::update_hist_entry(map<string, map<int, struct key_dist>>& key_hist, const string& prefix, size_t key_size, size_t value_size) {
    ...
    key_hist[prefix][key_slab].val_map[value_slab].max_len = std::max<size_t>(value_size, key_hist[prefix][key_slab].val_map[value_slab].max_len);
}
...
void BlueStore::_record_onode(OnodeRef& o, KeyValueDB::Transaction& txn) {
    ...
    dout(20) << __func__ << " onode " << o->oid << " is " << bl.length() << " (" << onode_part << " bytes onode + " << blob_part << " bytes spanning blobs + " << extent_part
...
void BlueStore::_record_allocation_stats() {
    ...
    auto t0 = std::make_tuple(alloc_stats_count.exchange(0), alloc_stats_fragments.exchange(0), alloc_stats_size.exchange(0));
...
bool BlueStoreRepairer::fix_shared_blob(KeyValueDB::Transaction txn, uint64_t sbid, bluestore_extent_ref_map_t* ref_map, size_t repaired) {
    string key;
    get_shared_blob_key(sbid, &key);
    if (ref_map) {
...
void RocksDBBlueFSVolumeSelector::dump(ostream& sout) {
    ...
    sout << "RocksDBBlueFSVolumeSelector: wal_total:" << l_totals[LEVEL_WAL - LEVEL_FIRST] << ", db_total:" << l_totals[LEVEL_DB - LEVEL_FIRST] << ", slow_total:" << l_totals[LEVEL_SLOW - LEVEL_FIRST]
